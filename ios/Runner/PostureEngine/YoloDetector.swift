...
/// same `yolov8n_float16.tflite` through TensorFlow Lite's Swift API.
///
...
final class YoloDetector {
    ...
    init?() {
        ...
        do {
            ...
            NSLog("\(TAG): failed to create interpreter: \(error)")
            return nil
        }
    }

    func detect(pixelBuffer: CVPixelBuffer) -> YoloResult {
        let input = buildInputTensor(pixelBuffer)
        do {
            try input.withUnsafeBytes { try interpreter.copy(Data($0), toInputAt: 0) }
            try interpreter.invoke()
            let output = try interpreter.output(at: 0)
            let floats = output.data.toFloatArray()
            return buildResult(parseAndNms(floats))
        } catch {
            NSLog("\(TAG): inference failed: \(error)")
            return YoloResult(personDetected: false, monitorDetected: false,
